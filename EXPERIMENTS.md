...
### E13-B. 정정: 타입별 T "부호 반전" 은 방향만 남는다 (2026-09-22, jev-exploration#10)
SamuelSacco 가 Gateway 를 거치지 않은 `api.typesafe.ai` 직접 응답 71개(숫자 3,180개)로 **0.01 양자화가 Jev 자체**임을
보였고(우리 "정답에 확률 0" 각주는 그대로 유효), 원시 primitive 별 끝점 사용률을 공개했다: Choice 는 70.4% 가 정확히 0,
Noul 은 2,580개 중 0/1 이 하나도 없음(0.01~0.98 clamp).

temperature 는 확률이 (0,1) 안쪽일 때만 정의된다. 우리 `eval_dump` 는 로그 바닥을 1e-6 으로 잘라서, 정답에 정확히 0 을
준 항목 하나가 13.8 nats 를 넣는다. E13 의 900건으로 재계산(`scripts/jev_endpoint_refit.py`):

| 타입 | 정확히 0 | 정확히 1 | 정답에 0 | T @1e-6 | **T @0.005** | T @0.01 | 정답-0 제외 @1e-6 |
|---|---|---|---|---|---|---|---|
| choice | 69.8% | 20.0% | 4.3% | 3.29 | **1.30** | 1.14 | 1.31 |
| score | 30.5% | 0.0% | 2.0% | 3.40 | **1.92** | 1.81 | 2.43 |
| noul | 0.0% | 0.0% | 0.0% | 0.66 | **0.66** | 0.66 | 0.66 |

- **방향은 살고 크기는 죽는다.** choice 의 과신은 4.3% 의 정답-0 항목이 만든 것이고, 끝점을 제대로 다루면 1.30 이다.
  score 는 1.92 로 여전히 과신(라벨이 텍스트에 없는 과제). noul 은 어떤 바닥값에서도 0.66 으로 동일.
- **정확도·NLL@1·Brier·ECE 는 영향 없다** (choice ECE 0.084 → 0.072). 움직이는 건 refit T 열뿐이다.
- **우리 프레이밍에 불리한 사실도 같이 적었다**: 우리가 "과소확신 극" 으로 쓴 Noul 이 바로 끝점에서 clamp 된 유일한
  타입이라, T 가 깨끗하게 정의되는 유일한 타입이다. 과신 쪽 두 타입은 그렇지 않다.
- 조치: `scienthoon/jev-ood-calibration@914d87a` 로 README 정정 + `scripts/endpoint_refit.py` 추가, 이슈에 답글.
- **규칙**: 로그 기반 지표(NLL, refit T)를 양자화·clamp 된 확률에 쓸 때는 바닥값 민감도를 함께 보고한다.
